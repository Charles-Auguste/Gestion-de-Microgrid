...
class Player:
    def __init__(self):
        ...
        self.demand = np.ones(self.horizon+1)
    ...
    def take_a_simple_decision(self):#on achète juste tout ce qui est demandé sans utiliser la batterie
        ...
        for t in range (self.horizon):
            cout += self.demand[t]*self.prices["purchase"][t]*self.dt
        return(cout)

    def set_scenario(self, scenario_data):
        self.demand = scenario_data
    ...
    def take_the_correct_decision(self):
        ...
        for t in range(self.horizon):
            ...
            lp += prod_vars_plus[t] + self.demand[t] + prod_vars_moins[t] <= sortieplus[t], constraintName
        lp.setObjective(self.dt * (pulp.lpSum([(self.prices["purchase"][t] * sortieplus[t] + (prod_vars_plus[t] + self.demand[t] + prod_vars_moins[t] - sortieplus[t]) * self.prices["sale"][t]) for t in range(self.horizon)])))
        ...
        for i in range(self.horizon):
            self.grid_relative_load[i] = Lbat[i] - self.demand[i]
        L2 = []
        for i in range(self.horizon):
...
def verification(player,grid_relative_load) :
    ...
    for t in range (player.horizon) :
        prod_vars_plus[t]=max(0, grid_relative_load[t]-player.demand[t])
        prod_vars_moins[t]=min(0, grid_relative_load[t]-player.demand[t])
        if (player.dt *
